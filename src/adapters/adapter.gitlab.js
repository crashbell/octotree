const
  ...
  , GL_RESERVED_REPO_NAMES = ['followers', 'following', 'repositories']
  , GL_BRANCH_SEL  = '[aria-label="Switch branches or tags"]'
  , GL_404_SEL          = '#parallax_wrapper'
  , GL_PJAX_SEL         = '#js-repo-pjax-container'
  , GL_CONTAINERS       = 'body > .page-with-sidebar'
 ...
 * Selects a path.
 */
GitLab.prototype.selectPath = function(path, tabSize) {
  var container = $(GL_PJAX_SEL)
 ...
 */
GitLab.prototype.getRepoFromPath = function(showInNonCodePage, currentRepo) {
  // 404 page, skip
  if ($(GL_404_SEL).length) return false
  ...
  if (~GL_RESERVED_USER_NAMES.indexOf(match[1])) return false
  if (~GL_RESERVED_REPO_NAMES.indexOf(match[2])) return false
  ...
  var branch =
    $(GL_BRANCH_SEL).data('ref') ||
    $(GL_BRANCH_SEL).children('.js-select-button').text() ||
